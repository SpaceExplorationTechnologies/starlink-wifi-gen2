		      ...
		      void *img, unsigned int img_len)
{
	...
	if (cert_nv_ctr < plat_nv_ctr) {
		/* Invalid NV-counter */
		NOTICE("Verifying NVCTR (FIP rollback version) ... FIP cert: %u, required: >=%u, Fail.\n",
		       cert_nv_ctr, plat_nv_ctr);
		return 1;
	} else if (cert_nv_ctr > plat_nv_ctr) {
		NOTICE("Verifying NVCTR (FIP rollback version) ... FIP cert: %u, required: >=%u, OK. Setting STSAFE\n",
		       cert_nv_ctr, plat_nv_ctr);
		rc = plat_set_nv_ctr(param->plat_nv_ctr->cookie, cert_nv_ctr);
		return_if_error(rc);
	} else { // if cert_nv_ctr == plat_nv_ctr
		NOTICE("Verifying NVCTR (FIP rollback version) ... FIP cert: %u, required: >=%u, OK.\n",
		       cert_nv_ctr, plat_nv_ctr);
	}
