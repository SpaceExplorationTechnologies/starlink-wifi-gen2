...
/* FIP 0 Policies */
static const struct plat_io_policy policies0[] = {
	[FIP_IMAGE_ID] = {
		&boot_dev_handle,
		(uintptr_t)&mtk_boot_dev_fip0_spec,
		check_boot_dev
	},
	[BL32_IMAGE_ID] = {
		&fip_dev_handle,
...
/* FIP 1 Policies */
static const struct plat_io_policy policies1[] = {
	[FIP_IMAGE_ID] = {
		&boot_dev_handle,
		(uintptr_t)&mtk_boot_dev_fip1_spec,
		check_boot_dev
	},
	[BL32_IMAGE_ID] = {
		&fip_dev_handle,
...
void bl2_platform_setup(void)
{
	...
	abort_boot_if_bl2_revoked();

	INFO("Loading FIP %d\n", stsafe_bootcount & 0x1);
}
			  ...
			  uintptr_t *image_spec)
{
	...
	if (stsafe_bootcount & 0x1) {
		assert(image_id < ARRAY_SIZE(policies1));
		policy = &policies1[image_id];
	} else {
		assert(image_id < ARRAY_SIZE(policies0));
		policy = &policies0[image_id];
	}
