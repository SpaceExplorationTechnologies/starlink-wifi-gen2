...
int scanResult(lua_State *L)
{
	...
	unsigned char *tmp_idx = luaL_checkstring(L, 2);
	struct iwreq wrq;
	unsigned char *data = NULL;
	unsigned int date_len = 5000;

	if((data = malloc(date_len)) == NULL){
		fprintf(stderr, "%s: malloc failed\n", __func__);
		return -1;
	}
	memset(data, 0, date_len);
	socket_id = socket(AF_INET, SOCK_DGRAM, 0);
	strcpy(wrq.ifr_name, interface);
	strncpy(data, tmp_idx, date_len-1);
	wrq.u.data.length = date_len;
