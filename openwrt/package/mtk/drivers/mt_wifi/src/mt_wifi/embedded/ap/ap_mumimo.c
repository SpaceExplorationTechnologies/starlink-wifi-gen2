...
)
{
	...
	param.group_index = (UINT16)cpu2le32(pParams->group_index);
...
)
{
	...
	param.u2GroupIndex = cpu2le32(pParams->group_index);
...
)
{
	...
	param.u2GroupIndex = (UINT16)(cpu2le32(mu_group->groupIndex));
...
*
*Description:
*	Set Falcon MURU Module Algo Parameter Ctrl
*
*Parameters:
*	Standard Falcon MURU Algo Parameter
*
*/

INT SetMuruAlgoCtrl(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	PCHAR pch = NULL;
	INT32 Ret = TRUE;
	/* prepare command message */
	struct _CMD_ATTRIBUTE attr = {0};
	struct cmd_msg *msg = NULL;
	UINT32 cmd = MURU_SET_ALGO_CTRL;
	CMD_MURU_ALGO_CTRL param = {0};

	pch = strsep(&arg, "-");

	if (pch != NULL)
		param.ucRuAlgoTimeOut = os_str_tol(pch, 0, 10);
	else {
		Ret = 0;
		goto error;
	}
	param.ucRuAlgoTimeOut = cpu2le32(param.ucRuAlgoTimeOut);
	msg = AndesAllocCmdMsg(pAd, sizeof(cmd) + sizeof(param));
...
INT SetMuruGlobalProtSecCtrl(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	*(UINT16 *)param = cpu2le16(*(UINT16*)param);
	msg = AndesAllocCmdMsg(pAd, sizeof(cmd) + sizeof(param));
...
INT SetMuruTxDataSecCtrl(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	*(UINT32 *)param = cpu2le32(*(UINT32*)param);
	msg = AndesAllocCmdMsg(pAd, sizeof(cmd) + sizeof(param));
...
int ShowMuruAlgoCtrl(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	INT32 Ret = TRUE;
	struct cmd_msg *msg = NULL;
	PCHAR pch = NULL;
	PCHAR pIndex = NULL;
	UINT32 index = 0;
	EVENT_SHOW_MURU_ALGO_CTRL result = {0};
	UINT32 cmd = MURU_GET_ALGO_CTRL;
	struct _CMD_ATTRIBUTE attr = {0};
...
static VOID muru_get_algo_ctrl_handler(struct cmd_msg *msg,
	char *rsp_payload, UINT16 rsp_payload_len)
{
	P_EVENT_MURU_GET_ALGO_CTRL pEntry = NULL;
	...
	pEntry = (P_EVENT_MURU_GET_ALGO_CTRL)rsp_payload;
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,
			 ("eventId %u\n", pEntry->u4EventId));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		("lq_report[0] = 0x%x\n", cpu2le32(pEntry->rEntry.ucRuAlgoTimeOut)));
}
							...
							UINT16 rsp_payload_len)
{
	...
	case MU_EVENT_HQA_GET_SU_LQ:
		...
		break;
#ifdef CFG_SUPPORT_FALCON_MURU
	case MURU_EVENT_GET_ALGO_CTRL:
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
				 ("%s: MURU_EVENT_GET_ALGO_CTRL\n", __func__));
		muru_get_algo_ctrl_handler(msg, pData, len);
		break;
