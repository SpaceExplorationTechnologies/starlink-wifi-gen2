...
#ifdef IXIA_SUPPORT
#define REORDERING_PACKET_TIMEOUT_IXIA		((500 * OS_HZ)/1000)	/* system ticks -- 100 ms*/
#define MAX_REORDERING_PACKET_TIMEOUT_IXIA	((5000 * OS_HZ)/1000)	/* system ticks -- 100 ms*/
#endif /*IXIA_SUPPORT*/
	...
	PVOID SystemSpecific3)
{
	...
	if ((pBAEntry->ORI_BA_Status == Originator_WaitRes) && (pBAEntry->Token < ORI_SESSION_MAX_RETRY)) {
		...
		MlmeEnqueueWithWdev(pAd, ACTION_STATE_MACHINE, MT2_MLME_ADD_BA_CATE, sizeof(MLME_ADDBA_REQ_STRUCT), (PVOID)&AddbaReq, 0, pEntry->wdev, NULL);
		...
		if (pBAEntry->REC_BA_Status != Recipient_NONE) {
			while ((mpdu_blk = ba_reordering_mpdu_dequeue(&pBAEntry->list))) {
				while ((msdu_blk = ba_reordering_mpdu_dequeue(&mpdu_blk->AmsduList))) {
					...
					ba_mpdu_blk_free(pAd, msdu_blk);
#ifdef IXIA_SUPPORT
					pAd->tr_ctl.tp_dbg.RxDropPacketCnt[DROP_MSDU_REL]++;
#endif /*IXIA_SUPPORT*/
				}
				...
				ba_mpdu_blk_free(pAd, mpdu_blk);
#ifdef IXIA_SUPPORT
				pAd->tr_ctl.tp_dbg.RxDropPacketCnt[DROP_MPDU_REL]++;
#endif /*IXIA_SUPPORT*/
			}
		}
	}
...
void ba_reordering_list_pkts_release(RTMP_ADAPTER *pAd)
{
	...
	for (i = 0; i < MAX_LEN_OF_BA_REC_TABLE; i++) {
		...
		if (pBAEntry->REC_BA_Status != Recipient_NONE) {
			...
			pBAEntry->LastIndSeqAtTimer = now;
		}
	}
	NdisReleaseSpinLock(&pAd->BATabLock);
...
void ba_timeout_monitor(PRTMP_ADAPTER pAd)
{
	...
	if (pAd->ixiaCtrl.iMode == VERIWAVE_MODE) {
		pAd->BATable.ba_flush_one_time = REORDERING_PACKET_TIMEOUT_IXIA;
		pAd->BATable.ba_flush_all_time = MAX_REORDERING_PACKET_TIMEOUT_IXIA;
	} else {
		pAd->BATable.ba_flush_one_time = REORDERING_PACKET_TIMEOUT;
		pAd->BATable.ba_flush_all_time = MAX_REORDERING_PACKET_TIMEOUT;
	}
#endif /*IXIA_SUPPORT*/
...

{
	...
	if (RTMP_TIME_AFTER((unsigned long)Now32,
		(unsigned long)(pBAEntry->LastIndSeqAtTimer + (pAd->BATable.ba_flush_all_time / 6)))
		&& (pBAEntry->list.qlen > 0)
	   ) {
		MTWF_LOG(DBG_CAT_PROTO, CATPROTO_BA, DBG_LVL_TRACE, ("timeout[%d] (%08lx-%08lx = %d > %d): %x, flush all!\n ",
			pBAEntry->list.qlen, Now32, (pBAEntry->LastIndSeqAtTimer),
			(int)((long) Now32 - (long)(pBAEntry->LastIndSeqAtTimer)), pAd->BATable.ba_flush_all_time,
			pBAEntry->LastIndSeq));
		ba_refresh_reordering_mpdus(pAd, pBAEntry);
		pBAEntry->LastIndSeqAtTimer = Now32;
	} else if (RTMP_TIME_AFTER((unsigned long)Now32,
	(unsigned long)(pBAEntry->LastIndSeqAtTimer + (pAd->BATable.ba_flush_one_time)))
			   && (pBAEntry->list.qlen > 0)
			  ) {
		/*
	...
	IN FRAME_ADDBA_RSP * pFrame)
{
	...
	if ((Idx != 0) && (pBAEntry->TID == TID) && (pBAEntry->ORI_BA_Status == Originator_WaitRes)) {
		...
		pBAEntry->amsdu_cap = pFrame->BaParm.AMSDUSupported && amsdu_en && cap->ppdu.SupportAMSDU;
...
VOID peer_addba_req_action(RTMP_ADAPTER *pAd, MLME_QUEUE_ELEM *Elem)
{
	...
	/* 2-1. Prepare ADDBA Response frame.*/
		...
		if (IS_ENTRY_PEER_AP(pMacEntry) || IS_ENTRY_REPEATER(pMacEntry)) {
			...
			if (pMacEntry && pMacEntry->bReptCli)
				...
				ActHeaderInit(pAd, &ADDframe.Hdr, pAddr, pAd->StaCfg[apidx].wdev.if_addr, pAddr);
		} else
#endif /* APCLI_SUPPORT */
#ifdef WDS_SUPPORT
	...
	if (pMacEntry && (Status == 0) && cap->ppdu.SupportAMSDU)
	...
	UCHAR wdev_idx)
{
	...
	if ((msdu_blk != NULL) &&
		...
		if (!RX_BLK_TEST_FLAG(pRxBlk, fRX_HDR_TRANS))
			...
			SET_OS_PKT_NETDEV(pRxBlk->pRxPacket, get_netdev_from_bssid(pAd, wdev_idx));
			SET_OS_PKT_DATATAIL(pRxBlk->pRxPacket, GET_OS_PKT_LEN(pRxBlk->pRxPacket));
		}
...
#endif
#ifdef IXIA_SUPPORT
				pAd->tr_ctl.tp_dbg.RxDropPacketCnt[ALREADY_IN_ORDER]++;
#endif /*IXIA_SUPPORT*/
...
#endif
#ifdef IXIA_SUPPORT
		pAd->tr_ctl.tp_dbg.RxDropPacketCnt[DROP_RX_NOT_ALLOW]++;
#endif /*IXIA_SUPPORT*/
...
#ifdef IXIA_SUPPORT
		pAd->tr_ctl.tp_dbg.RxDropPacketCnt[DROP_BA_STAT_ERROR]++;
#endif /*IXIA_SUPPORT*/
		RELEASE_NDIS_PACKET(pAd, pRxBlk->pRxPacket, NDIS_STATUS_FAILURE);
		return;
	}
...
#endif
#ifdef IXIA_SUPPORT
			pAd->tr_ctl.tp_dbg.RxDropPacketCnt[DUP_SEQ_PKT_EQ]++;
#endif /*IXIA_SUPPORT*/
			RELEASE_NDIS_PACKET(pAd, pRxBlk->pRxPacket, NDIS_STATUS_FAILURE);
		}
...
#endif
#ifdef IXIA_SUPPORT
		pAd->tr_ctl.tp_dbg.RxDropPacketCnt[DUP_SEQ_PKT_SMALL]++;
#endif /*IXIA_SUPPORT*/
		RELEASE_NDIS_PACKET(pAd, pRxBlk->pRxPacket, NDIS_STATUS_FAILURE);
...
#ifdef IXIA_SUPPORT
		if ((pAd->ixiaCtrl.iMode == VERIWAVE_MODE) && (pRxBlk->isNBDSPacket == TRUE)) {
			RELEASE_NDIS_PACKET(pAd, pRxBlk->pRxPacket, NDIS_STATUS_FAILURE);
			pAd->NbdsDelCnt++;
			return;
		}
	...
	IN PMAC_TABLE_ENTRY pEntry)
{
	...
	if (pEntry) {
		...
		RTMP_IO_WRITE32(pAd->hdev_ctrl, RegId, 0x4007); /* Legacy OFDM / no STBC / LGI / BW20 / MCS 7 */
