...
static VOID EFUSE_IO_READ32(PRTMP_ADAPTER pAd, UINT32 reg, UINT32 *value)
{
	...
	if (IS_HIF_TYPE(pAd, HIF_MT)) {
		...
		RTMP_IO_WRITE32(pAd->hdev_ctrl, MCU_PCIE_REMAP_2, MT_EEF_BASE - 0x70000);
		RTMP_IO_READ32(pAd->hdev_ctrl, 0x80000 + (reg - MT_EEF_BASE) + 0x70000, value);
...
static VOID EFUSE_IO_WRITE32(PRTMP_ADAPTER pAd, UINT32 reg, UINT32 value)
{
	...
	if (IS_HIF_TYPE(pAd, HIF_MT)) {
		...
		RTMP_IO_READ32(pAd->hdev_ctrl, MCU_PCIE_REMAP_2, &RestoreValue);
		RTMP_IO_WRITE32(pAd->hdev_ctrl, MCU_PCIE_REMAP_2, MT_EEF_BASE - 0x70000);
		RTMP_IO_WRITE32(pAd->hdev_ctrl, 0x80000 + (reg - MT_EEF_BASE) + 0x70000, value);
		RTMP_IO_WRITE32(pAd->hdev_ctrl, MCU_PCIE_REMAP_2, RestoreValue);
#endif
	} else
#endif /* MT_MAC */
