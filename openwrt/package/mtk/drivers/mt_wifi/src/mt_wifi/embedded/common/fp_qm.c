...
VOID fp_tx_pkt_deq_func(RTMP_ADAPTER *pAd)
{
	...
	while (need_schedule) {
		...
		if (wdev) {
			...
			pTxBlk->resource_idx = asic_get_resource_idx(pAd, wdev, 0, 0);
		...
		pTxBlk->HeaderBuf = asic_get_hif_buf(pAd, pTxBlk, pTxBlk->resource_idx, pTxBlk->TxFrameType);
#ifdef DSCP_PRI_SUPPORT
...
INT32 fp_enq_dataq_pkt(RTMP_ADAPTER *pAd, struct wifi_dev *wdev, PNDIS_PACKET pkt, UCHAR q_idx)
{
	...
	if (pAd->fp_que.Number >= DATA_QUE_MAX_NUMS)
		goto error;
