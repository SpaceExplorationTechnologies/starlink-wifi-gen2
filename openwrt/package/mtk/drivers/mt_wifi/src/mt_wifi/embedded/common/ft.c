	...
	IN INT32 PktLen)
{
	...
	/* SpaceX: Samsung Galaxy S9 sends malformed FT RRB pkts with a FTActLen greater than MAX_MGMT_PKT_LEN. 
	 * Since pOutBuffer is only allocated MAX_MGMT_PKT_LEN memory, the malformed pkts were causing buffer overflow and crashing the router.
	 * To prevent overflow, check if FTActLen is greater than the max mgmt pkt size. If it is, return and don't construct the outgoing frame. */
	if (pRrb->FTActLen > MAX_MGMT_PKT_LEN) {
		MTWF_LOG(DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_OFF,
				 ("SpaceX: %s FT RRB's ActLen is too large=%d. Don't construct outgoing frame.\n",
				  __func__, pRrb->FTActLen));
		return;
	}
