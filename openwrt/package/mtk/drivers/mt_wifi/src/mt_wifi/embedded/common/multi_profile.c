	...
	CHAR *final)
{
	...
	/* merge MinPhyRate */
	multi_profile_merge_perband(data, "MinPhyDataRate", buf1, buf2, final);
	multi_profile_merge_perband(data, "MinPhyBeaconRate", buf1, buf2, final);
	multi_profile_merge_perband(data, "MinPhyMgmtRate", buf1, buf2, final);
	multi_profile_merge_perband(data, "MinPhyBCMCRate", buf1, buf2, final);
	multi_profile_merge_perband(data, "LimitClientSupportRate", buf1, buf2, final);
	multi_profile_merge_perband(data, "DisableCCKRate", buf1, buf2, final);
	...
	CHAR *final)
{
	...
	for (ucRegionIdx = 0; ucRegionIdx < ucMaxNoOfRegion; ucRegionIdx++) {
		for (ucRTIdx = 0; ucRTIdx < RT_NUM; ucRTIdx++) {
			...
			if (RTMPGetKeyParameter(tok_str, tmpbuf, len, buf2, TRUE) == TRUE) {
				RTMPSetKeyParameter(tok_str, tmpbuf, len, final, TRUE);
				break;
			}
		}
	}

	if (RTMPGetKeyParameter("RadarPulseThresholdParam", tmpbuf, len, buf2, TRUE) == TRUE)
		RTMPSetKeyParameter("RadarPulseThresholdParam", tmpbuf, len, final, TRUE);
