...
bool RedMarkPktDrop(UINT8 ucWlanIdx, UINT8 ucQidx, PRTMP_ADAPTER pAd)
{
	...
	if (IsBadNode) {
		...
		if (pkt_buf_cnt >= (prRedSta->u4Dropth >> prAcElm->ucShiftBit)) {
			...
			if (pAd->red_debug_en) {
				pEntry = &pAd->MacTab.Content[ucWlanIdx];
				tx_rate.word = (USHORT)pEntry->LastTxRate;
				MTWF_LOG(DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_OFF,
					 ("(%s)STA %d, AC %d, Len %d, Thres %d, PHY(MODE %d, MCS %d, BW %d, ShortGI %d)\n",
					  __func__, ucWlanIdx, ucAC, pkt_buf_cnt,
					  prRedSta->u4Dropth >> prAcElm->ucShiftBit,
					  tx_rate.field.MODE, tx_rate.field.MCS,
					  tx_rate.field.BW, tx_rate.field.ShortGI));
			}
...
VOID red_record_data(PRTMP_ADAPTER pAd, UINT8 ucWlanIdx, PNDIS_PACKET pPacket)
{
	UINT8 ucAC;
