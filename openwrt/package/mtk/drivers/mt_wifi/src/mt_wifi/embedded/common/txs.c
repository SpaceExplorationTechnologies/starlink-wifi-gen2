...
INT32 NullFramePM1TxSHandler(RTMP_ADAPTER *pAd, CHAR *Data, UINT32 Priv)
{
	...
	MTWF_LOG(DBG_CAT_TX, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s, line(%d)\n", __func__, __LINE__));
	...
	if ((pAd->OpMode == OPMODE_STA) && (pAd->StaCfg[0].BssType == BSS_INFRA) && (pAd->StaCfg[0].WindowsPowerMode != Ndis802_11PowerModeCAM)) {
		if ((TxSD0->LE == 0) && (TxSD0->RE == 0) && (TxSD0->ME == 0)) {
			MTWF_LOG(DBG_CAT_TX, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Got TXS, OK, RTMPSendNullFrame(PM=1)\n"));
			RTMP_SLEEP_FORCE_AUTO_WAKEUP(pAd, pStaCfg);
		} else {
			MTWF_LOG(DBG_CAT_TX, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Got TXS, ERROR, Peer didn't get NullFrame(PM=1), LE(%d, RE(%d), ME(%d))\n",
					 TxSD0->LE,
					 TxSD0->RE,
					 TxSD0->ME));
			RTMP_SET_PSM_BIT(pAd, pStaCfg, PWR_ACTIVE);
#ifdef STA_LP_PHASE_2_SUPPORT
			RTMP_PWR_MGT_BIT_WIFI(pAd, pEntry->wcid, PWR_ACTIVE);
...
INT32 NullFramePM0TxSHandler(RTMP_ADAPTER *pAd, CHAR *Data, UINT32 Priv)
{
	...
	if ((pAd->OpMode == OPMODE_STA) && (pAd->StaCfg[0].BssType == BSS_INFRA) && (pAd->StaCfg[0].WindowsPowerMode != Ndis802_11PowerModeCAM)) {
		if ((TxSD0->LE == 0) && (TxSD0->RE == 0) && (TxSD0->ME == 0))
			MTWF_LOG(DBG_CAT_TX, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Got TXS, RTMPSendNullFrame(PM=0)\n"));
		else {
			MTWF_LOG(DBG_CAT_TX, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Got TXS, ERROR, Peer didn't get NullFrame(PM=1), LE(%d, RE(%d), ME(%d))\n",
					 TxSD0->LE,
					 TxSD0->RE,
					 TxSD0->ME));
		}
