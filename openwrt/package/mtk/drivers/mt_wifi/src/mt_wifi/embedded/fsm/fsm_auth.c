...
static VOID auth_fsm_msg_invalid_state(struct _RTMP_ADAPTER *pAd, MLME_QUEUE_ELEM *Elem)
{
	...
	if (cli_idx != NON_REPT_ENTRY) {
		...
		curr_state = pReptEntry->AuthCurrState;
	}
...
inline BOOLEAN auth_fsm_state_transition(struct wifi_dev *wdev, UCHAR cli_idx, ULONG next_state, const char *caller)
{
	ULONG old_state = 0;
	...
	if (cli_idx != NON_REPT_ENTRY) {
		...
		old_state = pReptEntry->AuthCurrState;
		pReptEntry->AuthCurrState = next_state;
	} else
#endif /* MAC_REPEATER_SUPPORT */
	{
...
{
	auth_fsm_state_transition(wdev, NON_REPT_ENTRY, AUTH_FSM_IDLE, __func__);
}
