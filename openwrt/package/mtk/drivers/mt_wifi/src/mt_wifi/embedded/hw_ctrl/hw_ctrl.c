...
#ifdef IXIA_SUPPORT
#ifdef VOW_SUPPORT
static NTSTATUS HwCtrlIXIA_VOW_UPDATE(RTMP_ADAPTER *pAd, HwCmdQElmt *CMDQelmt)
{
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s update airtime on/off.\n", __func__));
	vow_set_feature_all(pAd);
	return NDIS_STATUS_SUCCESS;
}
#endif /*VOW_SUPPORT*/
#ifdef RED_SUPPORT
static NTSTATUS HwCtrlIXIA_RED_ONOFF(RTMP_ADAPTER *pAd, HwCmdQElmt *CMDQelmt)
{
	UINT8 *redCtrl = (UINT8 *)CMDQelmt->buffer;

	if (*redCtrl == 1)
		set_red_enable(pAd, "1");
	else
		set_red_enable(pAd, "0");
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("RED %s.\n", (*redCtrl == 1) ? "ON" : "OFF"));
	return NDIS_STATUS_SUCCESS;
...
#ifdef IXIA_SUPPORT
#ifdef VOW_SUPPORT
	{HWCMD_ID_IXIA_CTRL_VOW, HwCtrlIXIA_VOW_UPDATE, 0},
#endif /*VOW_SUPPORT*/
#ifdef RED_SUPPORT
	{HWCMD_ID_IXIA_CTRL_RED, HwCtrlIXIA_RED_ONOFF, 0},
#endif /*RED_SUPPORT*/
#endif /*IXIA_SUPPORT*/
