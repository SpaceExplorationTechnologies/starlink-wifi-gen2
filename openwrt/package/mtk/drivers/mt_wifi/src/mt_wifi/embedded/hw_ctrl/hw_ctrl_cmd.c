...
#ifdef IXIA_SUPPORT
#ifdef VOW_SUPPORT
VOID RTMP_SET_IXIA_VOW_UPDATE(struct _RTMP_ADAPTER *pAd)
{
	UINT32 ret;
	HW_CTRL_TXD HwCtrlTxd;

	os_zero_mem(&HwCtrlTxd, sizeof(HW_CTRL_TXD));
	HW_CTRL_TXD_BASIC(pAd, HWCMD_TYPE_RADIO, HWCMD_ID_IXIA_CTRL_VOW, 0, NULL, HwCtrlTxd);
	ret = HwCtrlEnqueueCmd(pAd, HwCtrlTxd);
}
#endif /*VOW_SUPPORT*/
#ifdef RED_SUPPORT
VOID RTMP_SET_IXIA_RED_ON_OFF(struct _RTMP_ADAPTER *pAd, UINT8 onoff)
{
	UINT32 ret;
	HW_CTRL_TXD HwCtrlTxd;
	UINT8 redCtrl = 0;
	...
	os_zero_mem(&HwCtrlTxd, sizeof(HW_CTRL_TXD));
	redCtrl = onoff;
	HW_CTRL_TXD_BASIC(pAd, HWCMD_TYPE_RADIO, HWCMD_ID_IXIA_CTRL_RED, sizeof(UINT8), &redCtrl, HwCtrlTxd);
	ret = HwCtrlEnqueueCmd(pAd, HwCtrlTxd);
}
#endif /*RED_SUPPORT*/
#endif /*IXIA_SUPPORT*/