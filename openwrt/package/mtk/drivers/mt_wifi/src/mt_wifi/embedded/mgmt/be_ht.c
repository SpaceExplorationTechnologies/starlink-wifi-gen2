...
VOID operate_loader_trx_stream(struct wifi_dev *wdev, struct wlan_operate *op, UINT8 tx_stream, UINT8 rx_stream)
{
	...
	case 4:
		...
		op->ht_status.ht_cap_ie.MCSSet[3] = wdev->rate.EapMCSSet[3];
	    else
...
#ifdef CONFIG_RA_PHY_RATE_SUPPORT
	    if (wdev->rate.Eap_HtSupRate_En == TRUE)
		op->ht_status.ht_cap_ie.MCSSet[2] = wdev->rate.EapMCSSet[2];
	    else
#endif /* CONFIG_RA_PHY_RATE_SUPPORT */
...
#ifdef CONFIG_RA_PHY_RATE_SUPPORT
	    if (wdev->rate.Eap_HtSupRate_En == TRUE)
		op->ht_status.ht_cap_ie.MCSSet[1] = wdev->rate.EapMCSSet[1];
	    else
#endif /* CONFIG_RA_PHY_RATE_SUPPORT */
...
#ifdef CONFIG_RA_PHY_RATE_SUPPORT
	    if (wdev->rate.Eap_HtSupRate_En == TRUE)
		op->ht_status.ht_cap_ie.MCSSet[0] = wdev->rate.EapMCSSet[0];
	    else
#endif /* CONFIG_RA_PHY_RATE_SUPPORT */
