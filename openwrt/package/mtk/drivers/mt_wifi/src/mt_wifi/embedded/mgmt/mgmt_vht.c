...
INT build_vht_oper_ie(RTMP_ADAPTER *pAd, UCHAR bw, UCHAR Channel, struct wifi_dev *wdev, UCHAR *buf)
{
	...
	/* BW 160M or 80M+80M */
	if ((wlan_config_get_vht_bw(wdev) == 2 || wlan_config_get_vht_bw(wdev) == 3)
		&& wlan_config_get_ht_bw(wdev) == 1
		&& max_vht_mcs == VHT_MCS_CAP_9) {
		switch	(wlan_operate_get_rx_stream(wdev)) {
		case 4:
...
INT build_vht_cap_ie(RTMP_ADAPTER *pAd, struct wifi_dev *wdev, UCHAR *buf)
{
	...
	if ((mcs_cap <= VHT_MCS_CAP_9)
		...
		} else if (wlan_config_get_vht_bw(wdev) == 0
			&& wlan_config_get_ht_bw(wdev) == 1) {
			/* BW 40M */
		...
		} else if ((wlan_config_get_vht_bw(wdev) == 2 || wlan_config_get_vht_bw(wdev) == 3)
			&& wlan_config_get_ht_bw(wdev) == 1) {
			/* BW 160M or 80M+80M */
