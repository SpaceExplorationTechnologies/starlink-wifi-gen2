...
int main(int argc, char *argv[])
{
	...
	while (chipset != NULL) {
		...
		if ((strncmp(chipset, "mt7636u", 7) == 0)
			...
			if ((strncmp(fpga, "y", 1) == 0)) {
				printf("HAS_FPGA_MODE(y)::mt7637_patch_e1_e2_hdr_FPGA.bin\n");
				strcat(in_rom_patch, "mt7637_patch_e1_hdr_FPGA.bin");
				strcat(in_rom_patch_e2, "mt7637_patch_e2_hdr_FPGA.bin");
			} else {
				printf("HAS_FPGA_MODE(n)::mt7637_patch_e1_e2_hdr.bin\n");
				strcat(in_rom_patch, "mt7637_patch_e1_hdr.bin");
				strcat(in_rom_patch_e2, "mt7637_patch_e2_hdr.bin");
			}

			strcat(out_rom_patch, "mt7637_rom_patch.h");
			strcat(rom_patch_name, "mt7637_rom_patch");
			strcat(out_rom_patch_e2, "mt7637_rom_patch_e2.h");
			strcat(rom_patch_name_e2, "mt7637_rom_patch_e2");
			...
			if ((strncmp(chipset, "mt7637u", 7) == 0))
				strcat(ine2pname, "MT7657U_E2_QFN_eFuse.bin");
			else if ((strncmp(chipset, "mt7637e", 7) == 0))
				strcat(ine2pname, "MT7657E_E2_QFN_eFuse.bin");
			else
				strcat(ine2pname, "MT7657S_E2_QFN_eFuse.bin");
			...
			is_bin2h_rom_patch = 1;
                        ...
                        is_bin2h_e2p = 1;
                        is_bin2h_rom_patch = 1;
		} else if (strncmp(chipset, "mt7663", 7) == 0) {
			...
			strcat(in_rom_patch, "rebb/axe_patch_e1_hdr.bin");
