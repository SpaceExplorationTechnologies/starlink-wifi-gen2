...
VOID MtAsicSetBssid(RTMP_ADAPTER *pAd, UCHAR *pBssid, UCHAR BssidIndex)
{
	...
	val = (UINT32)((pBssid[0]) |
				   ...
				   (UINT32)(pBssid[2] << 16) |
				   (UINT32)(pBssid[3] << 24));
	MAC_IO_WRITE32(pAd->hdev_ctrl, curr_bssid_reg_base + (BssidIndex * 8), val);
	val = (UINT32)(pBssid[4]) | (UINT32)(pBssid[5] << 8) | (1 << 16);
	MAC_IO_WRITE32(pAd->hdev_ctrl, (curr_bssid_reg_base + 4) + (BssidIndex * 8), val);
	/* If we enable BSSID0, we should not enable MBSS0, or the BSSID index will wrong */
	/* MAC_IO_READ32(pAd->hdev_ctrl, RMAC_ACBEN, &val); */
	/* val |= 0x1; */
	/* MAC_IO_WRITE32(pAd->hdev_ctrl, RMAC_ACBEN, val); */
}
...
INT MtAsicWtblSetRDG(RTMP_ADAPTER *pAd, BOOLEAN bEnable, UINT8 Wcid)
{
	...
	HW_IO_READ32(pAd->hdev_ctrl, tb_entry.wtbl_addr[0] + (2 * 4), &dw2->word);
	...
	HW_IO_WRITE32(pAd->hdev_ctrl, tb_entry.wtbl_addr[0] + (2 * 4), dw2->word);
	return TRUE;
}
...
INT MtAsicSetRDG(RTMP_ADAPTER *pAd, BOOLEAN bEnable, UCHAR BandIdx)
{
	...
	MAC_IO_READ32(pAd->hdev_ctrl, TMAC_TCR, &tmac_tcr);
	MAC_IO_READ32(pAd->hdev_ctrl, AGG_PCR, &agg_pcr);
	MAC_IO_READ32(pAd->hdev_ctrl, TMAC_TRCR, &tmac_trcr);
	...
	MAC_IO_WRITE32(pAd->hdev_ctrl, TMAC_TCR, tmac_tcr);
	MAC_IO_WRITE32(pAd->hdev_ctrl, AGG_PCR, agg_pcr);
	MAC_IO_WRITE32(pAd->hdev_ctrl, TMAC_TRCR, tmac_trcr);
	return TRUE;
}
...
VOID MtAsicUpdateRtsThld(PRTMP_ADAPTER pAd, MT_RTS_THRESHOLD_T *rts_thrld)
{
	...
	MAC_IO_READ32(pAd->hdev_ctrl, AGG_PCR1, &Value);
	...
	MAC_IO_WRITE32(pAd->hdev_ctrl, AGG_PCR1, Value);
}
	...
	IN UINT32 apidx)
{
	...
	case BCN_FLUSH: {
		...
		while (1) {
			...
			MAC_IO_READ32(pAd->hdev_ctrl, cr_base, &temp);
...
VOID MtAsicAddRemoveKeyByDriver(PRTMP_ADAPTER pAd, ASIC_SEC_INFO *pInfo)
{
	...
	HW_IO_READ32(pAd->hdev_ctrl, tb_entry.wtbl_addr[0], &dw0->word);
	HW_IO_READ32(pAd->hdev_ctrl, tb_entry.wtbl_addr[0]+4, &dw1->word);
	HW_IO_READ32(pAd->hdev_ctrl, tb_entry.wtbl_addr[0]+8, &dw2->word);
	dw0->field.wm = 0;
...
#endif /* RT_BIG_ENDIAN */
		HW_IO_WRITE32(pAd->hdev_ctrl, addr+index, *pKeyData);
	}
...
INT32 MtAsicGetAntMode(RTMP_ADAPTER *pAd, UCHAR *AntMode)
{
	...
	HW_IO_READ32(pAd->hdev_ctrl, STRAP_STA, &Value);
...
BOOLEAN MtAsicGetMcuStatus(RTMP_ADAPTER *pAd, MCU_STAT State)
{
	...
	HW_IO_READ32(pAd->hdev_ctrl, TOP_MISC2, &value);
...
#ifdef CONFIG_STA_SUPPORT
	arch_ops->archEnableIbssSync = MtAsicEnableBssSyncByDriver;
#endif
