...
VOID PDMAResetAndRecovery(RTMP_ADAPTER *pAd)
{
	...
	HIF_IO_READ32(pAd->hdev_ctrl, MT_WPDMA_GLO_CFG, &Value);
	Value |= FORCE_TX_EOF;
	HIF_IO_WRITE32(pAd->hdev_ctrl, MT_WPDMA_GLO_CFG, Value);
	/* Infor PSE client of TX abort */
	MAC_IO_READ32(pAd->hdev_ctrl, MCU_PCIE_REMAP_2, &RestoreValue);
	RemapBase = GET_REMAP_2_BASE(RST) << 19;
	RemapOffset = GET_REMAP_2_OFFSET(RST);
	MAC_IO_WRITE32(pAd->hdev_ctrl, MCU_PCIE_REMAP_2, RemapBase);
	MAC_IO_READ32(pAd->hdev_ctrl, 0x80000 + RemapOffset, &Value);
	Value |= TX_R_E_1;
	MAC_IO_WRITE32(pAd->hdev_ctrl, 0x80000 + RemapOffset, Value);
	...
	Value &= ~TX_R_E_2;
	MAC_IO_WRITE32(pAd->hdev_ctrl, 0x80000 + RemapOffset, Value);
	MAC_IO_WRITE32(pAd->hdev_ctrl, MCU_PCIE_REMAP_2, RestoreValue);
...
#endif
	RTMP_IO_READ32(pAd->hdev_ctrl, 0x816c, &Value);
	Value |= (1 << 0);
	RTMP_IO_WRITE32(pAd->hdev_ctrl, 0x816c, Value);
...
BOOLEAN PciMonitorRxPse(RTMP_ADAPTER *pAd)
{
	...
	if (pAd->RxPseCheckTimes < 10) {
		...
		MAC_IO_WRITE32(pAd->hdev_ctrl, 0x4244, 0x98000000);
		MAC_IO_READ32(pAd->hdev_ctrl, 0x4244, &Value);
		...
		MAC_IO_WRITE32(pAd->hdev_ctrl, MCU_PCIE_REMAP_2, RemapBase);
		MAC_IO_WRITE32(pAd->hdev_ctrl, 0x80000 + RemapOffset, 3);
		MAC_IO_READ32(pAd->hdev_ctrl, 0x80000 + RemapOffset, &Value);
