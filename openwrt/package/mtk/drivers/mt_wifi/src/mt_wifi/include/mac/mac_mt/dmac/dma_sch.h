...
#define HIF_DMASHDL_IO_READ32(_A, _R, _pV) HW_IO_READ32(_A->hdev_ctrl, (0x6000+((UINT32)_R&0xfff)), _pV)
#define HIF_DMASHDL_IO_WRITE32(_A, _R, _pV) HW_IO_WRITE32(_A->hdev_ctrl, (0x6000+((UINT32)_R&0xfff)), _pV)
#else
#define HIF_DMASHDL_IO_READ32(_A, _R, _pV) \
	do {\
		...
		HW_IO_READ32(_A->hdev_ctrl, (0x80000+((UINT32)_R&0xffff)), _pV);\
		\
...
#define HIF_DMASHDL_IO_WRITE32(_A, _R, _pV) \
	do {\
		...
		\
		HW_IO_WRITE32(_A->hdev_ctrl, 0x2504, DMASHDL_BASE);\
		HW_IO_WRITE32(_A->hdev_ctrl, (0x80000+((UINT32)_R&0xffff)), _pV);\
		\
		HW_IO_WRITE32(_A->hdev_ctrl, 0x2504, restore_remap_addr);\
	} while (0)
#endif
