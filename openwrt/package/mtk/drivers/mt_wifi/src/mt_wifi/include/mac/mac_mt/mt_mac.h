...
INT mt_ct_get_hw_resource_free_num(struct _RTMP_ADAPTER *pAd, UCHAR resource_idx, UINT32 *free_num, UINT32 *free_token);
INT mt_ct_hw_tx(struct _RTMP_ADAPTER *pAd, struct _TX_BLK *tx_blk);
#ifdef REDUCE_TX_OVERHEAD
INT mt_ct_hw_tx2(struct _RTMP_ADAPTER *pAd, struct _TX_BLK *tx_blk);
#endif
