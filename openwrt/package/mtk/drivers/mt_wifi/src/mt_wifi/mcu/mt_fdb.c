...
UINT32 HostAccessFwCr(RTMP_ADAPTER *pAd, UINT32 fw_cr_addr, UINT32 cr_op, UINT32 wrt_val)
{
        ...
        if (IS_MT7615(pAd) || IS_MT7622(pAd))
        {
				...
				HW_IO_READ32(pAd->hdev_ctrl, MCU_PCIE_REMAP_1, &ori_remap_cr_val);
                /* do PCI-E remap for physical base address to 0x40000 */
				HW_IO_WRITE32(pAd->hdev_ctrl, MCU_PCIE_REMAP_1, fw_cr_addr);
                /* get real remap cr base addr */
				HW_IO_READ32(pAd->hdev_ctrl, MCU_PCIE_REMAP_1, &rec_remap_cr_base_addr);
                ...
                if ((fw_cr_addr - rec_remap_cr_base_addr) > REMAP_1_OFFSET_MASK)
                {
						...
						HW_IO_WRITE32(pAd->hdev_ctrl, MCU_PCIE_REMAP_1, ori_remap_cr_val);
                }
                target_cr_offset = ((fw_cr_addr - rec_remap_cr_base_addr) & REMAP_1_OFFSET_MASK);

                if (cr_op == CR_OP_READ) {
					RTMP_IO_READ32(pAd->hdev_ctrl, 0x40000 + target_cr_offset, &remapped_cr_val);
					...
					RTMP_IO_WRITE32(pAd->hdev_ctrl, 0x40000 + target_cr_offset, wrt_val);
                }
