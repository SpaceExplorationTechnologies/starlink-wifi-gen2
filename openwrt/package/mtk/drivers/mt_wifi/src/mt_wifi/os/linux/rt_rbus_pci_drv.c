...
NDIS_STATUS mt_pci_chip_cfg(RTMP_ADAPTER *pAd, USHORT  id)
{
	...
	if ((id == NIC7603_PCIe_DEVICE_ID)
		...
		RTMP_IO_READ32(pAd->hdev_ctrl, TOP_HVR, &Value);
		pAd->HWVersion = Value;

		if (Value == 0)
			ret = NDIS_STATUS_FAILURE;

		RTMP_IO_READ32(pAd->hdev_ctrl, TOP_FVR, &Value);
		pAd->FWVersion = Value;
		...
		if (IS_MT7603(pAd)) {
			RTMP_IO_READ32(pAd->hdev_ctrl, STRAP_STA, &Value);
...
#ifdef CONFIG_WIFI_MSI_SUPPORT
{
	...
	pci_hif->is_msi = ((struct pci_dev *)pci_dev)->msi_enabled;
}
