...
static VOID ge_combo_tx_pkt_deq_tasklet(ULONG param)
{
	RTMP_ADAPTER *pAd = (RTMP_ADAPTER *)param;

	ge_combo_tx_pkt_deq_func(pAd);
}
...
#ifdef DYNAMIC_STEERING_LOADING
	RTMP_OS_TASKLET_INIT(pAd, &pAd->tx_combo_deque_tasklet, ge_combo_tx_pkt_deq_tasklet, (unsigned long)pAd);
	pAd->combo_tx_dequeue_scheduable = TRUE;
#endif /* DYNAMIC_STEERING_LOADING */
	return ret;
}
...
static VOID rx2_done_tasklet(unsigned long data)
{
	PRTMP_ADAPTER pAd = (PRTMP_ADAPTER)data;
...
static INT tm_tasklet_hif_pci_init(RTMP_ADAPTER *pAd)
{
	...
	if (IS_MT7626_FW_VER_E2(pAd) && IS_ASIC_CAP(pAd, fASIC_CAP_MD)) {
		RTMP_OS_TASKLET_INIT(pAd, &obj->rx_done_task[HIF_RX_IDX2], rx2_done_tasklet, (unsigned long)pAd);
	}
#endif
#ifdef ERR_RECOVERY
...
#ifdef DYNAMIC_STEERING_LOADING
	pAd->combo_tx_dequeue_scheduable = FALSE;
	RTMP_OS_TASKLET_KILL(&pAd->tx_combo_deque_tasklet);
#endif /* DYNAMIC_STEERING_LOADING */
...
#ifdef MT7626_E2_SUPPORT
	if (IS_MT7626_FW_VER_E2(pAd) && IS_ASIC_CAP(pAd, fASIC_CAP_MD)) {
		RTMP_OS_TASKLET_KILL(&obj->rx_done_task[HIF_RX_IDX2]);
	}
#endif
#ifdef ERR_RECOVERY
...
static INT tm_tasklet_qm_schedule_task(RTMP_ADAPTER *pAd, enum task_type type)
{
	...
	case TX_DEQ_TASK:
		if (pAd->tx_dequeue_scheduable) {
#ifdef DYNAMIC_STEERING_LOADING
			if (pAd->tasklet_schdule_lo_flag)
				RTMP_OS_TASKLET_SCHE_LO(&pAd->tx_deque_tasklet);
			else
#endif
...
#ifdef DYNAMIC_STEERING_LOADING
	case TX_COMBO_DEQ_TASK:
		if (pAd->combo_tx_dequeue_scheduable) {
#ifdef DYNAMIC_STEERING_LOADING
			if (pAd->tasklet_schdule_lo_flag)
				RTMP_OS_TASKLET_SCHE_LO(&pAd->tx_combo_deque_tasklet);
			else
#endif /* DYNAMIC_STEERING_LOADING */
...
#ifdef DYNAMIC_STEERING_LOADING
		if (pAd->tasklet_schdule_lo_flag)
			RTMP_OS_TASKLET_SCHE_LO(&obj->tx_dma_done_task);
		else
#endif /* DYNAMIC_STEERING_LOADING */
...
#ifdef DYNAMIC_STEERING_LOADING
		if (pAd->tasklet_schdule_lo_flag)
			RTMP_OS_TASKLET_SCHE_LO(&obj->tr_done_task);
		else
#endif /* DYNAMIC_STEERING_LOADING */
...
#ifdef DYNAMIC_STEERING_LOADING
		if (pAd->tasklet_schdule_lo_flag)
			RTMP_OS_TASKLET_SCHE_LO(&obj->rx_done_task[HIF_RX_IDX2]);
		else
#endif /* DYNAMIC_STEERING_LOADING */
			RTMP_OS_TASKLET_SCHE(&obj->rx_done_task[HIF_RX_IDX2]);
		break;
