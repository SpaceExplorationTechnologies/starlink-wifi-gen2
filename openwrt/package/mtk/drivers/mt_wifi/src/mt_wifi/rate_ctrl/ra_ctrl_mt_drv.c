...
)
{
	...
	if (pRaParam->u4Field == RA_PARAM_VHT_OPERATING_MODE) {
		...
		} else if (pRaParam->u4Field == RA_PARAM_VERIWAVE_RVR_PER) {
			pCmdStaRecAutoRateUpdate->u4Field = RA_PARAM_VERIWAVE_RVR_PER;
	...
	UINT32 u4RegValue)
{
	...
	struct cmd_msg *msg = NULL;
	struct _CMD_ATTRIBUTE attr = {0};
	CMD_SET_AGG_LIMIT_T param;

	os_move_mem(&param.eap_agglimit, peap_agglimit, sizeof(EAP_AGG_LIMIT_SET));
	msg = AndesAllocCmdMsg(pAd, sizeof(param));
	if (!msg) {
		ret = -1;
		goto error;
	...
	SET_CMD_ATTR_MCU_DEST(attr, HOST2N9);
	SET_CMD_ATTR_TYPE(attr, EXT_CID);
	param.u4EapCtrlCmdId = SET_AGG_LIMIT;
	...
	struct _EAP_ACKTIMEOUT_SET *peap_acktimeout)
{
	...
	CMD_SET_ACKTIMEOUT_T param;

	os_move_mem(&param.eap_acktimeout, peap_acktimeout, sizeof(EAP_ACKTIMEOUT_SET));
	msg = AndesAllocCmdMsg(pAd, sizeof(param));
	if (!msg) {
		ret = -1;
		goto error;
	...
	SET_CMD_ATTR_MCU_DEST(attr, HOST2N9);
	SET_CMD_ATTR_TYPE(attr, EXT_CID);
	param.u4EapCtrlCmdId = SET_ACKTIMEOUT;
	SET_CMD_ATTR_EXT_TYPE(attr, EXT_CMD_ID_EAP_CTRL);
	SET_CMD_ATTR_CTRL_FLAGS(attr, INIT_CMD_SET);
	SET_CMD_ATTR_RSP_EXPECT_SIZE(attr, MT_IGNORE_PAYLOAD_LEN_CHECK);
	SET_CMD_ATTR_RSP_WB_BUF_IN_CALBK(attr, NULL);
