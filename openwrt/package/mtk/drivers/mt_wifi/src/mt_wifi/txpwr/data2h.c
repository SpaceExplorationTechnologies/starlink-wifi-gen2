	...
	while (1) {
		...
		case FILE_TRANSFORM_PARSING_STATE:
			if (c == ',') {
				...
				if (c == '\n') {
					/* new line symbol handle which is converted to '\t' */
					fputs("\t\"\n\"", outfile);
				} else if (c == '\r') {
					/* skip return symbol */
					continue;
		...
		case FILE_TRANSFORM_CHECK_STATE:
			if (c != ',') {
				...
				if (c == '\r') {
					...
					/* replace comma symbol with space symbol for non-continuous comma */
					cTempStringBuffer = ' ';
					sprintf(cString, "%c", cTempStringBuffer);
					fputs(cString, outfile);
...
void FilePathArrangeProc(char *pcFilePath, int u4FilePathSize, char *EnvironDirectoryPath, char *DirectoryPath, int u4Type, int u4PwrLimitType, int u4PwrLimitTblIdx)
{
	...
	case OUTPUT_FILE:
		strcat(pcFilePath, "PowerLimit.h");
		printf("Output: %s\n", pcFilePath);
		break;
