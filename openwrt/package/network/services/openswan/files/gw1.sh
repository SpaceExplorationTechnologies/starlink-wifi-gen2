...
#ifconfig br-lan 10.10.40.254 netmask 255.255.255.0
#ifconfig $WAN_IF 10.10.20.253 netmask 255.255.255.0
route add default gw 10.10.20.254


iptables -I FORWARD -s 192.168.2.0/24 -p tcp --tcp-flags SYN,RST SYN -j TCPMSS --
iptables -I FORWARD -d 192.168.1.0/24 -p tcp --tcp-flags SYN,RST SYN -j TCPMSS --
iptables -I FORWARD -s 192.168.2.0/24 -p tcp --tcp-flags SYN,RST SYN  -j TCPMSS -
iptables -I FORWARD -d 192.168.1.0/24 -p tcp --tcp-flags SYN,RST SYN  -j TCPMSS -
#iptables -t nat -F
iptables -t nat -I POSTROUTING -o $WAN_IF -s 192.168.2.0/24 -j ACCEPT
...
echo "@LEFT @RIGHT : PSK \"123456789\"" > /etc/ipsec.secrets
