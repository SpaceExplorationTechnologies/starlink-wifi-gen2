...
./bch i "$FLASH_NAME" "$PRELOADER_NAME" "$PRELOADER_NAME".pad 0 0 "$PRELOADER_PAD"
./bch i "$FLASH_NAME" "$PRELOADER_NAME".pad "$PRELOADER_NAME".img 1 64 0

# ATF's size is 0x40000, image size should be 2 block
./bch i "$FLASH_NAME" "$ATF_NAME" "$ATF_NAME".pad 0 0 "$ATF_PAD"

# Uboot/Config/RF partition has 0x80000 + 0x80000 + 0x40000 = 0x140000
# so UBOOT Need to pad to 10 blocks
./bch i "$FLASH_NAME" "$UBOOT_NAME" "$UBOOT_NAME".pad 0 0 "$UBOOT_PAD"
